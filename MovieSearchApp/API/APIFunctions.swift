...
func searchMovies(searchQuery: String, year: String? = nil, pageNum: Int = 1) async -> SearchResponse? {
    ...
    do {
        let response = try await AF.request(apiRequestString).serializingData().result.get()
        do {
            let response: SearchResponse = try JSONDecoder().decode(SearchResponse.self, from: response)
            return response
...
func loadMoreResults(existingResponse: SearchResponse, query: String, numPages: Int) async -> SearchResponse {
    ...
    for pageCounter in (combinedResponse.numPagesLoaded+1)...(combinedResponse.numPagesLoaded+numPages) {
...
func fetchMovie(title: String, year: String) async -> Movie? {
    ...
    do {
        let response = try await AF.request(apiRequestString).serializingData().result.get()
        do {
            let movie: Movie = try JSONDecoder().decode(Movie.self, from: response)
            return movie
        } catch DecodingError.dataCorrupted(let context) {
            print(context)
        } catch DecodingError.keyNotFound(let key, let context) {
